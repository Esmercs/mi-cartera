...
check('24% anual → 2.32% mensual (IVA incluido)', monthlyRate(24), 0.0232, 1e-9)
check('0% → 0', monthlyRate(0), 0, 1e-9)

console.log('accruedInterest')
check('saldo 100000 a 24% (2% + IVA)', accruedInterest(100000, 24), 2320)
...
check('100000 a 24% en 24 meses', amortizedPayment(100000, 24, 24), 5480.70)
...
check('24 meses con la cuota calculada', projectPayoff(100000, 24, 5480.70).months, 24, 0)
check('saldo 0 → 0 meses', projectPayoff(0, 24, 5480.70).months, 0, 0)
check('interés restante a 24 meses', projectPayoff(100000, 24, 5480.70).remainingInterest, 31536.80, 1)
const sinExtra = projectPayoff(100000, 24, 5480.70)
const conExtra = projectPayoff(80000, 24, 5480.70)
...
const movs = [
  ...
  { kind: 'interest',     amount: 2320,   effective_date: '2026-01-31', accrual_month: '2026-01-01' },
  { kind: 'payment',      amount: 5480.70, effective_date: '2026-02-05' },
]
check('saldo al arranque', balanceAsOf(movs, '2026-01-01'), 100000)
check('saldo con el interés del mes', balanceAsOf(movs, '2026-01-31'), 102320)
check('saldo después del abono', balanceAsOf(movs, '2026-02-28'), 96839.30)
...
check('interés devengado de enero', splitInterest(movs, '2026-01'), 2320)
